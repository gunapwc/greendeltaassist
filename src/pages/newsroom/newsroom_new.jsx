...
import {
  NewsRoomCard,
  NewsRoomBanner,
} from "../../component/newsroom/newsroomCard";
...
const NewsRoom1 = () => {
  ...
  const { loading, error, data } = useQuery(News_Room, {
    variables: { id: 0 },
  });
        ...
        {content.map((value, index) => {
          if (index > 2 && index < 5) {
            return (
              <NewsRoomCard
                key={index}
                image={value.thumb_image_full_path}
                date={value.created_at}
                category={value.category_name}
                title={value.news_title}
                click={() => {
                  onclickCard(value.news_id);
                }}
              />
            );
          }
    ...
    <>
      <div className="container">
        ...
        <NewsRoomBanner
          ...
          category="Health"
          title="Enhance your beauty & improve joint health with Country Life Maxi- Collagen C&A+ Biotin Powder"
        />
